...
const TaskBreakdown = ({ data }) => {
  ...
  const totals = useMemo(() => {
    ...
    Object.values(taskBreakdown).forEach(tasks => {
      ...
      totalHours += tasks.reduce((sum, task) => sum + (task.hours || 0), 0);
    });
    
    return { totalTasks, totalHours };
  }, [taskBreakdown]);

  // Calcular distribución por dueño
  const ownerDistribution = useMemo(() => {
    ...
    Object.values(taskBreakdown)
      ...
      .forEach(task => {
        if (task.owner) {
          distribution[task.owner] = (distribution[task.owner] || 0) + (task.hours || 0);
        }
      });
    return distribution;
  }, [taskBreakdown]);

  // Calcular timeline por día
  const timelineData = useMemo(() => {
    if (!data?.sprintDuration) return [];
    
    const days = [];
    for (let day = 1; day <= data.sprintDuration; day++) {
      const dayTasks = Object.values(taskBreakdown)
        .flat()
        .filter(task => task.day === day);
      const dayHours = dayTasks.reduce((sum, task) => sum + (task.hours || 0), 0);
      days.push({ day, hours: dayHours });
    }
    return days;
  }, [taskBreakdown, data?.sprintDuration]);

  // Helper function para colores de dueño - mover arriba para usarla en el componente
  const getOwnerColor = (owner) => {
    ...
    return colors[owner] || colors['Default'];
  };
  ...
  return (
    <div className="task-breakdown">
      <div className="breakdown-header">
        <h2>🔨 Task Breakdown - Sprint {data.sprintNumber || 1}</h2>
      ...
      <div className="breakdown-container">
        {data.selectedItems.map(item => {
          ...
          const itemTotalHours = tasks.reduce((sum, task) => sum + (task.hours || 0), 0);
          
          // Calcular progreso general del item
            ...
            <div key={item.id} className="breakdown-item">
              ...
              >
                ...
                <div className="header-right">
                  ...
                  <span className="points-badge">{item.points || 0} points</span>
                </div>
              </div>
                    ...
                    <tbody>
                      {tasks.map((task, index) => (
                        <tr key={index}>
                          ...
                          <td>
                            <span className="hours-tag">{task.hours || 0}h</span>
                          </td>
                          <td>
                            <select 
              ...
              {isExpanded && tasks.length === 0 && (
                <div className="item-tasks">
                  <p className="text-muted">No hay tareas definidas para este item.</p>
                  <button className="btn btn-sm btn-outline">
                    ➕ Add First Task
                  </button>
                </div>
      ...
      <div className="breakdown-summary">
        <div className="summary-section">
          ...
          <div className="owner-distribution">
            {Object.entries(ownerDistribution).map(([owner, hours]) => (
              <div key={owner} className="owner-dist">
                ...
                <div className="dist-bar-container">
                  <div 
                    className="dist-bar" 
                    style={{ 
                      width: totals.totalHours > 0 ? `${(hours / totals.totalHours) * 100}%` : '0%',
              ...
              <div key={day} className="timeline-day">
                ...
                <div className="day-bar-container">
                  <div 
                    className="day-bar" 
                    style={{ 
                      height: `${Math.min((hours / 8) * 100, 100)}%`,
                      backgroundColor: hours > 8 ? '#ef4444' : '#10b981'
                    }}
                    title={`${hours} hours`}
                  >
